...
@Service
class DictionaryRenamedSendMessageFactory(
        private val messageTemplateProvider: MessageTemplateProvider,
) {
    fun getInstance(chatId: Long, oldName: String, name: String): SendMessage {
        val msg = MessageFormat(messageTemplateProvider.getMessage(chatId, MessageCodes.MESSAGE_DICTIONARY_RENAMED))
                .format(arrayOf(oldName.escape(), name.escape()))
